...
//    }
    ...
    public function DirectIncomeWithdraw() {
        ...
        if (is_logged_in()) {
            ...
            $response['des'] = "Minimum Transfer Amount ZNX 1";
            $response['user'] = $this->User_model->get_single_record('tbl_users', array('user_id' => $this->session->userdata['user_id']), '*');
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    $todayWithdraw = $this->User_model->get_single_record('tbl_withdraw', array('user_id' => $this->session->userdata['user_id'],'date(created_at)' => date('Y-m-d')), '*');
                    $withdraw_amount = $this->input->post('amount');
                    ...
                    if(empty($todayWithdraw)){
                        if ($withdraw_amount >= 1 && $withdraw_amount <= 1000) {
                            //if($user['retopup'] == 0){
                                    ...
                                    // if ($user['master_key'] == $master_key) {
                                        ...
                                        // if ($data['pin_transfer'] == 0) {
                                            $withdrawArr = array(
                                                ...
                                                'fund_conversion' => ($withdraw_amount - ($withdraw_amount * 10 / 100))*$response['tokenValue']['amount'],
                        ...
                        // }
                            ...
                            $this->session->set_flashdata('message', 'Minimum Withdrawal Amount is '.currency.' 1 and Maximum Amount '.currency.' 1000');
                        }
                    } else {
                        $this->session->set_flashdata('message', '<h5 class="text-danger">Once a withdraw in a day,Please try tomorrow</h5>');
                    }
                    // } else {
