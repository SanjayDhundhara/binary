...
class AjaxController extends CI_Controller {
    ...
    public function jsonData(){
        if(is_logged_in()){
            $response['freeTeam'] = $this->User_model->calculateTeam($this->session->userdata['user_id'],0);
            $response['paidTeam'] = $this->User_model->calculateTeam($this->session->userdata['user_id'],1);
            $response['paidDirects'] = $this->User_model->get_single_record('tbl_users', ['sponser_id' => $this->session->userdata['user_id'],'paid_status' => 1], 'ifnull(count(id),0) as paidDirects');
            $response['freeDirects'] = $this->User_model->get_single_record('tbl_users', ['sponser_id' => $this->session->userdata['user_id'],'paid_status' => 0], 'ifnull(count(id),0) as freeDirects');
            echo json_encode($response);
